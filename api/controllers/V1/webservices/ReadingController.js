...
module.exports = {
  ...
  supplierListing: function (req, res) {
    ...
    reflikedSuppliers.orderByChild('user_id').equalTo('-L0UbCoAiFk06mBEYfDZ').on("value", function (snapshot) {
      ...
      Object.keys(favouritesSupplires).forEach(key = > {if(favouritesSuppliresId.indexOf(favouritesSupplires[key].supplier_id) === -1
    )
      {
    ...
    refSuppliers.orderByKey().on("value", function (snapshot) {
      ...
      Object.keys(suppliers).forEach(key = > {var supplierWithDistance = {};
