...
module.exports = {
  ...
  validate: function (req) {
    ...
    if (req.param('latitute') != undefined) {
      if (validator.isEmpty(req.param('latitute')) || validator.isEmpty(req.param('longitute'))) {
        errors['area'] = {message: User.message.area_required}
      } else if (isNaN(req.param('latitute')) || isNaN(req.param('longitute')) || req.param('latitute') < -90 || req.param('latitute') > 90 || req.param('longitute') < -180 || req.param('longitute') > 180) {
        errors['area'] = {message: User.message.area_length}
      }
    }
